...
_env = gym.make('MountainCar-v0', render_mode='rgb_array')
seed_everything(_env)
...
if use_saved_model:
    q_network.load_state_dict(torch.load('sec8_q_network.pth'))
    target_q_network.load_state_dict(torch.load('sec8_target_q_network.pth'))
...
def deep_sarsa(q_network, policy, episodes, alpha=0.001, batch_size=32, gamma=0.99, epsilon=0.05):
    ...
    for episode in tqdm(range(1, episodes + 1)):
        ...
        stats['MSE Loss'].append(sum(loss_list) / len(loss_list))
...
if not use_saved_model:
    stats = deep_sarsa(q_network, policy, 1500, epsilon=0.01, batch_size=128)

    # torch.save(q_network.state_dict(), 'sec8_q_network.pth')
    # torch.save(target_q_network.state_dict(), 'sec8_target_q_network.pth')
